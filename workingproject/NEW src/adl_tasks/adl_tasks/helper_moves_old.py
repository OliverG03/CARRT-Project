...
class MoveItHelper:
    ...
    # [FLAG:start-state-sanitize] treat controller-reported angles beyond +/-pi as wrapped
    # equivalents when publishing request.start_state.
    START_STATE_WRAP_THRESHOLD_RAD = math.pi + 0.05
    ...
    def _sanitize_start_state_positions(self, name_to_pos: dict, context: str) -> list[float] | None:
        ...
        for joint_name in self.ARM_JOINT_NAMES:
            ...
            if pos is None or (not math.isfinite(float(pos))):
                ...
                return None
            pos_f = float(pos)
            if abs(pos_f) > float(self.START_STATE_WRAP_THRESHOLD_RAD):
                wrapped_val = self._canonicalize_joint_angle(pos_f)
                wrapped.append((joint_name, pos_f, wrapped_val))
        ...
        if wrapped:
            wrapped_desc = ", ".join(
                [f"{jn}:{old:+.3f}->{new:+.3f}" for jn, old, new in wrapped]
            )
            action_desc = (
                "publishing canonicalized start_state"
                if self.START_STATE_CANONICALIZE_WRAPPED_JOINTS
                else "keeping raw wrapped start_state"
            )
            self.node.get_logger().warn(
                f"{context}: raw /joint_states contained wrapped arm joints beyond "
                f"+/-{self.START_STATE_WRAP_THRESHOLD_RAD:.3f}rad: {wrapped_desc}; "
                f"{action_desc}."
            )
        return sanitized
