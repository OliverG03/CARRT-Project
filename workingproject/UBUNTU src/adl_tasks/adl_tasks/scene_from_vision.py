...
# [FLAG scene-heartbeat-republish] Even if a tag pose is steady, republish that object's
...
class SceneFromVisionNode(Node):
    ...
    def _publish_from_cache(self):
        ...
        for tag_id in publish_set:
            ...
            publish_reason = "new"
            # [FLAG scene-heartbeat-republish] A pure "pose unchanged => never publish again" rule
    ...
    def _make_collision_object(
        ...
        *,
        ...
        def _side_face_normal_xy(tag_normal, tag_center_x, tag_center_y):
            # [FLAG scene-side-face-sign] Side tags should point toward the actual wrist camera.
        ...
        def _set_upright_yaw_from_axis(target_pose: Pose, yaw_axis):
            # [FLAG scene-upright-manual-z] Force side-grasp objects to stay upright in world Z and
        ...
        def _set_flat_qr_up_from_tag(target_pose: Pose, tag_axis_for_object_x):
            # [FLAG scene-flat-manual-z] Force top-grasp objects to lie flat with world +Z as the
        ...
        def _set_horizontal_cylinder_from_tag(target_pose: Pose, length_axis_hint):
            # [FLAG scene-bottle-horizontal-manual-z] The fallen bottle is a top-grasp object, but
...
def main(args=None):
    ...
    try:
        ...
        node.destroy_node()
        # [FLAG safe-shutdown] Multi-node bringup in VBox is routinely interrupted with Ctrl-C.
