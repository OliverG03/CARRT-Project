...
class VisionClient:
    ...
    def scan_scene(self, timeout_s: float = 6.0):
    ...
    def clear_scene_memory(self, timeout_s: float = 4.0) -> bool:
    ...
    def _call_pose_client(self, client, tag_id: int):
    ...
    def get_tag_pose(self, tag_id: int):
        if self.scene_pose_client.service_is_ready():
            pose, resp = self._call_pose_client(self.scene_pose_client, tag_id)
            if pose is not None:
                return pose
            self.node.get_logger().warn(
        ...
        pose, resp = self._call_pose_client(self.client, tag_id)
        if pose is not None:
            return pose
    ...
    def get_live_tag_pose(self, tag_id: int):
        pose, resp = self._call_pose_client(self.client, tag_id)
        if pose is not None:
            return pose
        self.node.get_logger().warn(
