...
from std_msgs.msg import Int32MultiArray, Bool, String  # for tag detection results (tag_id, x, y, z)
...
STUB_POSES = {
    ...
    1: make_pose(
        # [FLAG stub-medication-pose] Restore the medication bottle pose in the main stub so
        # give_medication can be exercised in the same default stub launch as the other ADLs.
...
class VisionStubNode(Node):
    ...
    def _publish_medication_name(self):
        # [FLAG stub-medication-name] Mirror the simple medication-name stub used in the older PDB
        # test harness so give_medication can validate both the bottle pose and QR-read stage under
        # the default vision stub.
        if not self._medication_qr_read_active:
            return
        if self._scene_locked or not self._vision_enabled:
            return
