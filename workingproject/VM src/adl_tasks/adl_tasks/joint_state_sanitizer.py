...
class JointStateSanitizer(Node):
    ...
    def _stamp_and_publish(self, msg: JointState) -> None:
        # [FLAG sanitized-fresh-timestamps] MoveItHelper rejects sanitized joint states older than
    ...
    def _publish_latest_sanitized(self) -> None:
        ...
        if self._latest_raw_version != self._last_published_raw_version:
            # [FLAG sanitized-downsample-live] MoveIt does not need the full hardware joint-state
        ...
        if (now - self._last_publish_time) < float(self.SANITIZED_IDLE_REPUBLISH_PERIOD_S):
            return
        # [FLAG sanitized-idle-refresh] Once the raw stream goes quiet, keep a slower keepalive
    ...
    def _on_joint_state(self, msg: JointState) -> None:
        ...
        # [FLAG sanitized-source-of-truth] Cache the latest normalized state here and let the fixed
...
def main(args=None) -> None:
    ...
    try:
        ...
        node.destroy_node()
        # [FLAG safe-shutdown] Jazzy can already shut down the default context on Ctrl-C before
