...
DROP_CONFIG = {
    ...
    "stage6_bin_reorient_trigger_gap": 0.10, # [FLAG stage6-bin-hazard-range] A 20cm trigger was firing while the remote was still far above the holder; keep BIN rescue earlier than shelves, but not so early that it interrupts a still-safe drop.
    # [FLAG stage6-lock-warn-ratio] Only trigger ratio-based rescue when the joint has EXCEEDED
    # its lock window (ratio > 1.0). A locked Cartesian step that fully succeeds cannot produce
    # ratio > 1.0 by construction (the constraint was satisfied during planning), so values below
    # 1.0 represent normal IK drift within the allowed window and should not abort the descent.
    # Setting to 1.10 provides a small execution-error buffer above the theoretical ceiling.
    "stage6_reorient_lock_warn_ratio": 1.10,
    "stage6_reorient_pull_up_z": 0.03,
    "stage6_reorient_max_retries": 1,
    "stage6_reorient_enable": True, # [FLAG stage6-generic-reorient] Make the pull-up/reorientation rescue available to any placement task that uses the shared Stage 6 helper, not only BIN drops in clear_table.
    "stage6_rescue_on_failed_descent": True, # [FLAG stage6-generic-failed-step] If a vertical descent dead-ends, allow one reorientation retry before failing the place attempt outright.
...
GIVE_MEDICATION_CONFIG = {
    ...
    "handover_stepwise_enable": True, # [FLAG medication-stepwise-handover] Lower medication in guarded vertical steps first so the arm can stop above the table instead of committing to one long final drop.
    ...
    "handover_step_rescue_enable": True, # [FLAG medication-stepwise-rescue] If the handover lower dead-ends, allow one pull-up/retry before falling back to direct pose moves.
...
def cartesian_descend_stepwise(
    ...
    *,
    ...
    # [FLAG stage6-near-goal-hazard-release] If a descent reaches the normal early-release
    ...
    while z_cur - z_goal > 1e-4:
        ...
        for subdiv in range(DROP_CONFIG["descent_subdivides"] + 1):
            ...
            # [FLAG stage6-unlock-track] Track whether this step's success came from the
    ...
    *,
    ...
    while True:
        ...
        if safe_joint_target:
            # [FLAG stage6-shared-joint-restore] When a task captured a safe above-slot branch,
            ...
            if rescue_align_ok and refreshed is not None:
                ...
                if preset_pose is not None:
                    ...
                    if (
                        ...
                        # [FLAG stage6-joint-restore-validate] A restored joint branch is only useful
        ...
        if active_joint_locks:
            ...
            if latest_joints:
                # [FLAG stage6-reanchor-locks] After a rescue, the drop should preserve the new rescued
