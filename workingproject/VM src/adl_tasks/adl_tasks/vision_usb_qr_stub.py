...
class VisionUsbQrStubNode(Node):
    ...
    def _init_apriltag_backend(self):
        # [FLAG usb-apriltag-backend-fallback] Prefer pupil_apriltags for parity with the real
    ...
    def _open_camera_source(self):
        # [FLAG usb-camera-source-probe] VirtualBox often exposes the webcam at a different
    ...
    def _poll_camera(self):
        ...
        if now_s - self._last_frame_ok_log_s >= 5.0:
            # [FLAG usb-camera-heartbeat] Log a low-rate heartbeat so you can tell the node is
        ...
        if not found_any:
            if now_s - self._last_no_detection_log_s >= 2.0:
                # [FLAG usb-no-detection-log] Give a throttled visible-state log directly from the
    ...
    def _log_visible_state(self):
        ...
        # [FLAG usb-visible-id-log] Emit the currently visible IDs and coarse scene poses from the
...
def main(args=None):
    ...
    try:
        ...
        node.destroy_node()
        # [FLAG safe-shutdown] Ctrl-C can invalidate the default ROS context before node cleanup.
