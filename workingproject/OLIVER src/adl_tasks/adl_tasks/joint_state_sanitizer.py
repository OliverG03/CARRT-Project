...
class JointStateSanitizer(Node):
    ...
    def _normalize_joint_angle_for_moveit(self, angle_rad: float) -> float:
        ...
        if abs(canonical) > max_mag:
            # [FLAG sanitizer-sign-preserve] Preserve the sign of the RAW input when
            # clamping near +/-pi.  atan2 wraps (-pi - eps) to (+pi - eps), which
            # flips the sign and creates a ~2*pi discontinuity between the value MoveIt
            # sees and the value the controller reports.  Using the raw sign ensures the
            # clamped result stays on the same side of the boundary as the physical
            # joint, preventing MoveIt error -26 (invalid start state).
