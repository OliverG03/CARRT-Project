...
class TaskBase:
    ...
    def _on_emergency_stop(self, msg: Bool):
        if msg.data and self.executing:
            ...
            self._cancel_reason = "Emergency stop activated. Motion halted. Click Turn Off to park the arm at retract."
            self._best_effort_stop_motion(timeout=0.5)
            self.publish_status(STATUS_CANCELLED, self._cancel_reason)

    def _best_effort_stop_motion(self, timeout: float = 0.5) -> None:
        arm = getattr(self.node, "arm", None)
        if arm is None or not hasattr(arm, "stop_motion"):
            return
        try:
            arm.stop_motion(timeout=timeout)
        except Exception as exc:
            try:
                self.node.get_logger().warn(f"{self.node_name}: stop_motion on cancel failed: {exc}")
            except Exception:
                pass
    ...
    def request_cancel(self, reason: str, detail: str | None = None):
        # Normal task-stop requests should mark only the active task as
    ...
    def publish_status(self, status: str, detail: str = ""):
        ...
        try:
            ...
            # Late task-thread updates can race node teardown.
    ...
    def _run_task(self, task_fn):
        try:
            self.reset_cancel()
            # Startup/idle parking is owned by the shared ADL controller.
            ...
            self.publish_status(STATUS_RUNNING, "Task started.")
            task_fn()
            ...
            if self._terminal_status in {STATUS_CANCELLED, STATUS_FAILED, STATUS_SUCCEEDED}:
                # Task-specific code already decided the final outcome.
                # Do not replace it with a generic success just because the worker function returned.
                return
            self.publish_status(STATUS_SUCCEEDED, "Task completed successfully.")
        except Exception as e:
            self.node.get_logger().error(f"{self.node_name}: task execution: {e}")
            import traceback
            self.node.get_logger().error(traceback.format_exc())
            self.publish_status(STATUS_FAILED, f"Task failed with exception: {e}")
        finally:
            self.executing = False
            # Do not auto-publish IDLE at task end. Keep the task's
