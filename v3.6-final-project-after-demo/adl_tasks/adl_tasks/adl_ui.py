...
# This file keeps the newer Brielle UI layout while preserving
# the VM build's current ROS topics and safer startup behavior.
# Adds Turn Off command to park robot in retract pose.
...
class ADLUINode(Node):
    def __init__(self):
        ...
        # The current VM build should not move the real arm
...
def patient_name():
    ...
    if _ui_node and value:
        # give_medication waits on /patient_name_entered, so the UI must
...
def status_json():
    # The browser page is otherwise static after the POST response. Poll the current
